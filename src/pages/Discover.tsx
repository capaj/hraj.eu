...
export const Discover: React.FC<DiscoverProps> = ({ onViewEvent }) => {
  ...
  const filteredAndSortedEvents = useMemo(() => {
    ...
    return sorted;
  }, [mockEvents, selectedSport, selectedSkillLevel, sortBy, userLocation]);
  ...
  const handleEventSelect = (event: Event) => {
    ...
    if (eventElement) {
      ...
      eventElement.classList.add('ring-2', 'ring-primary-500', 'ring-opacity-50');
      setTimeout(() => {
        eventElement.classList.remove('ring-2', 'ring-primary-500', 'ring-opacity-50');
      }, 2000);
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-white/80 mt-2">Find and join sports events in your area</p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            >
              <option value="date" className="text-gray-900">Date & Time</option>
              <option value="distance" className="text-gray-900">Distance</option>
              <option value="spots" className="text-gray-900">Available Spots</option>
            </select>
          </div>
        </div>
        ...
        {/* Map View */}
        <div className="mb-8">
          <div className="bg-white rounded-xl shadow-sm border border-white/20 overflow-hidden" style={{ height: '400px' }}>
            <EventMap
        ...
        <div>
          <div className="flex items-center mb-6">
            ...
            <h2 className="text-xl font-semibold text-white">
              Event List {sortBy !== 'date' && `(sorted by ${getSortLabel(sortBy)})`}
            </h2>
          </div>
          ...
          {filteredAndSortedEvents.length > 0 ? (
                    ...
                    {sortBy !== 'date' && (
                      <div className="absolute -top-2 -left-2 z-10">
                        <div className="bg-white text-primary-600 text-xs px-2 py-1 rounded-full font-medium shadow-lg">
                          {sortBy === 'distance' && distance ? distance : 
                           sortBy === 'spots' ? `${event.maxParticipants - event.participants.length} spots` : 
                           `#${index + 1}`}
                        </div>
                      </div>
                    )}
            ...
            <div className="text-center py-12">
              ...
              <p className="text-white/80 mb-4">Try adjusting your filters or create a new event.</p>
